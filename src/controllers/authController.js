...
class AuthController {
  ...
  login = asyncHandler(async (req, res) => {
    ...
    if (user.role === 'coach' && user.coachStatus === 'rejected') {
      const Notification = require('../models/Notification'); // تأكد من المسار الصحيح لموديل الإشعارات عندك
      const lastRejectionNotification = await Notification.findOne({
        userId: user._id,
        type: 'COACH_REJECTED',
      }).sort({ createdAt: -1 });

      const message = lastRejectionNotification?.message 
        ? lastRejectionNotification.message 
        : 'Your coach request has been rejected';

      const resp = error(message, 403);
      return res.status(resp.status).json(resp);
    }
